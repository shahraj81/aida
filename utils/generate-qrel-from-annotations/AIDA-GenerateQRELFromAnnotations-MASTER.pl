...
### DO NOT INCLUDE
use GenerateQRELFromAnnotationsManagerLib;
use POSIX;

### DO INCLUDE
##################################################################################### 
# This program generates QREL from LDC's annotations. 
##
...
my $switches = SwitchProcessor->new($0, "Generate QREL from LDC's annotations",
				    						"");
...
$switches->addParam("P103_ent_mentions", "required", "P103_ent_mentions.tab file");
...
foreach my $path(($switches->get("P103_ent_mentions"),
...
my $P103_ent_mentions_filename = $switches->get("P103_ent_mentions");
my $entity_mentions = EntityMentions->new($logger, $P103_ent_mentions_filename);
...
my $sentence_boundaries = SentenceBoundaries->new($logger, $sentence_boundaries_filename);

print $program_output "KBID\tCLASS\tID\tMODALITY\tDOCID\tSPAN\tCORRECTNESS\tTYPE\tFQEC\n";
my $filehandler = FileHandler->new($logger, $pool_filename);
my $entries = $filehandler->get("ENTRIES");
foreach my $entry($entries->toarray()) {
	my ($nodeid, $modality, $docid, $span) 
		= map {$entry->get($_)}
			qw(KBID MODALITY DOCID SPAN);
	my $fqec = $entity_mentions->get("FQEC", $sentence_boundaries, $nodeid, $modality, $docid, $span);
	my $label = "Incorrect";
	$label = "Correct" if $fqec;
	print "$nodeid\tNIL\t<ID>\t$modality\t$docid\t$span\t$label\tNIL\t$fqec\n";
}
