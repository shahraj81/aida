...
sub new {
  my ($class, $logger, $filename, $field_separator) = @_;
...
sub load {
  ...
  $self->{HEADER} = Header->new($self->get("LOGGER"), $line, $self->get("FIELD_SEPARATOR"));
  ...
  while($line = <FILE>){
    ...
    my $entry = Entry->new($self->get("LOGGER"), $linenum, $line, $self->{HEADER}, 
                {FILENAME => $filename, LINENUM => $linenum}, $self->get("FIELD_SEPARATOR"));
